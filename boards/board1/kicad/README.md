# Board 1 KiCad project — complete schematic and initial PCB placement
...
Completed 2026-09-07 with KiCad 7.0.11, native schematic format20230121, revision A-draft. Open [board1.kicad_pro](board1.kicad_pro). The editable source is the authority for implemented connectivity. The [native PCB](board1.kicad_pcb) now has all components approximately placed, with no tracks, vias or zones. See [placement notes and checks](placement.md) and the [preview](review/board1-initial-placement.png). The outline and placement are provisional; no fabrication outputs have been created.
...
| 4 | [Gateway](gateway.kicad_sch) | STM32G473, supplies, crystal, reset/BOOT0, SWD and timing/debug |
...
| 11 | [Access](access.kicad_sch) | External terminal block, three TVS arrays, six ground headers, twelve test pads and seven communications headers |

[Complete 11-page review PDF](review/board1-complete.pdf). The earlier two-page power/USB PDF and capture_manifest.json are retained as historical capture evidence; the complete PDF is also an older capture snapshot. Use native schematic files and [complete_manifest.json](complete_manifest.json) for current review. The revised access page is available as [SVG](review/board1-access.svg).

## Capture and component reconciliation

239 schematic component objects comprise 227 fitted components/connectors and 12 PCB copper test pads; two removable CAN shunts are assembly accessories without additional PCB footprints. The strip-stock purchase and testpad/shunt exceptions are explicit in [schematic_bom_allocations.json](schematic_bom_allocations.json). All populated components have MPNs, references and footprint assignments in [bom_internal.csv](../bom_internal.csv); no count remains TBD. Evidence status stays unverified until the relevant approval checks finish.
...
PCB spacing follow-up: the SAM timing headers are farther from the terminal,
the lower IC groups use more of the lower-right area, and every testpad has
function silkscreen and nearby ground access. See [test access](placement.md#test-access).
...
Run `python3 scripts/check_board1_complete.py` from the repository root. It exports native KiCad XML and checks239 objects/879 pins, explicit NCs, fields, exact numbered-pad coverage, all BOM quantities and independent topology rules for MCU ownership, SWD, header branches, ring, UART_MD, CAN polarity/termination/protection, RS-485, separate SAM core rails and preserved power/USB circuitry. The fresh PCB/netlist comparison and independent topology checks pass. The full schematic/BOM check now stops at the preexisting B059 purchased-header MPN mismatch; see placement notes. The older PDF was rendered and reviewed at its original capture stage. Capture scripts are one-time provenance, not the editing authority; do not rerun over subsequent manual CAD edits.

ERC has not run: KiCad7 CLI exposes schematic export but no ERC command. Run Eeschema ERC before schematic approval. PCB DRC was run through the Python API for [initial placement](placement.md#checks-performed); it is not a clean fabrication DRC. Exact assembly/mating, detailed placement and routing remain layout checks. No hardware validation is claimed. The stale ADR-040 research link now points to its verified historical Git revision; the accepted decision text is unchanged. The current document and purchasing audits flag the preexisting B027/B059 purchase reconciliation; see ../digikey_upload_review.md.
...
ADR-046 communications access: J19/J20 CAN H,L; J21 RS-485 A,B; J22/J23/J24 ring01/ring12/ring20 signal,GND; J25 UART_MD signal,GND. All are fitted 1×2 0.1-inch headers with function and pin-order silkscreen. TP1–TP12 remain copper pads.
