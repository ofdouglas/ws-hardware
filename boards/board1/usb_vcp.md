...
Status: selected baseline with implementation checks open. Authority: ADR-001/005/013/015/018/022/023/024/027/028/030/031. See [USB input](usb_input.md) for the attachment ramp, buck enable circuit and current model; this note does not reinstate earlier converter or supervisor proposals.
...
SN74LV125APWR B1-B016 is powered by 3V3_SYS and uses all four channels for TX/RX/RTS/CTS, two in each direction. This is power-off signal isolation, not galvanic isolation. Establish receiver defaults and /OE behavior during reset, main-rail ramp, suspend and brownout; part selection alone does not prove these states safe. Do not require application firmware to enable the ROM-programming path. Verify thresholds, leakage and propagation at 12 Mbaud with short gateway/FTDI routes (B1-Q002/008).
