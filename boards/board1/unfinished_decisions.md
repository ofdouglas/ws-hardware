...
| B1-Q002/008 | VCP buffer enables and receiver defaults | Implement accepted direct PWREN# /OE control (ADR-040); complete receiving-domain defaults. No rail monitors or delayed enable are required. Preserve all four VCP signals. B1-B016 uses all four channels. See [clocking](clocking.md) and the [enable proposal](research/vcp_enable_proposal.md). |
...
| B1-Q002/004/011 | USB shield and power operating envelope | Decide shield bonding and document ambient, permitted timing-header load, representative worst-case traffic and startup/suspend assumptions. Reconcile pull-up/bias loads and final capacitor counts into the power model. See [USB input](usb_input.md). |
...
Complete bridge support, receiver defaults and ordinary reset/boot circuits during capture. Track unresolved connectivity explicitly; do not delay drawing settled circuitry until all implementation details or bench tests are complete.
